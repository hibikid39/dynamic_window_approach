...
class IdealRobot:   
    def __init__(self, pose, agent=None, sensor=None, color="black", goal=None):    # 引数を追加
        self.pose = pose
        self.r = 0.2  
        self.color = color 
        self.agent = agent
    ...
    def draw(self, ax, elems):         ### call_agent_draw
        ...
        if self.agent and hasattr(self.agent, "draw"):                               #以下2行追加   
            self.agent.draw(ax, elems, self.poses[-1])
    ...
    def one_step(self, time_interval):
        ...
        nu, omega = self.agent.decision(obs, self.pose, time_interval) #引数追加
...
class Map:
    ...
    def append_landmark(self, landmark, g):       # ランドマークを追加
        if g == True:
            landmark.id = 100 # ゴールにはID 100を与える
        else:
...
class IdealCamera:
    ...
    def visible(self, polarpos):  # ランドマークが計測できる条件
        ...
        return polarpos[0] <= self.distance_range[1] and self.direction_range[0] <= polarpos[1] <= self.direction_range[1]
        
    def data(self, cam_pose):
        observed = []
        for lm in self.map.landmarks:
            z = self.observation_function(cam_pose, lm.pos)
            if self.visible(z) or lm.id == 100:               # 条件を追加
                #observed.append((z, lm.id))   # インデント
                observed.append((z, lm))
